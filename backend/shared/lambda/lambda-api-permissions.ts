...
import {SharedAuthorizerLambdaConstruct} from "./shared-authorizer-lambda";
import {UserManagementLambdaConstruct} from "./user-management-lambda";
import {TenantManagementLambdaConstruct} from "./tenant-management-lambda";
import {SharedApiConstruct} from "../api";

type LambdaApiPermissionsConfig = {
    variable: core.VariableResult;
    api: SharedApiConstruct;
...
export class LambdaApiPermissionsConstruct extends Construct {
    ...
    constructor(scope: Construct, id: string, config: LambdaApiPermissionsConfig) {
        ...
        new awsProvider.lambdaPermission.LambdaPermission(this, 'authorizer_permission', {
            functionName: config.authorizerLambda.resource.lambdaFunctionNameOutput,
            ...
            statementId: `${statementIdPrefix}-${config.authorizerLambda.resource.lambdaFunctionNameOutput}`,
            action,
            principal,
        ...
        new awsProvider.lambdaPermission.LambdaPermission(this, 'create_tenant_permission', {
            functionName: config.tenantManagementLambda.createTenantLambda.lambdaFunctionArnOutput,
            sourceArn: `${config.api.resource.apigatewayv2ApiExecutionArnOutput}/*/POST/tenant`,
            statementId: `${statementIdPrefix}-${config.tenantManagementLambda.createTenantLambda.lambdaFunctionNameOutput}`,
            action,
            principal,
        ...
        const registerAccessRolePolicy = new awsProvider.iamPolicy.IamPolicy(this, 'register_access_role_policy', {
            name: `saas-register-access-role-policy-${config.variable.region.value}-${config.variable.environment.value}`,
                ...
                Statement: [
                    {
                        ...
                        Action: ['execute-api:*'],
                        Resource: [
                            `${config.api.resource.apigatewayv2ApiExecutionArnOutput}/*/POST/tenant`,
                            `${config.api.resource.apigatewayv2ApiExecutionArnOutput}/*/POST/user/tenant-admin`
                        ]
                    }
                ]
            }),
        ...
        new awsProvider.iamRolePolicyAttachment.IamRolePolicyAttachment(this, 'register_access_role_policy_attachment', {
            role: config.tenantManagementLambda.registerLambdaAccessRole.name,
            policyArn: registerAccessRolePolicy.arn,

            dependsOn: [registerAccessRolePolicy, config.tenantManagementLambda.registerLambdaAccessRole]
        });
    }
}