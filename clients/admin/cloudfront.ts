...
export class CloudfrontConstruct extends Construct {
    ...
    constructor(scope: Construct, id: string, config: CloudfrontConfig) {
        ...
        const adminClientFullDomainName = `${config.variable.adminClientSubDomainName.value}.${config.variable.domainName.value}`;
        const certificate = new Certificate(this, 'certificate', {
            domainName: config.variable.domainName.value,
            zoneId: config.zone.zoneId,
            subjectAlternativeNames: [
                `${adminClientFullDomainName}`,
                `*.${adminClientFullDomainName}`
            ],
        ...
        const adminCloudfront = new Cloudfront(this, 'resource', {
            aliases: [`${core.getSubDomainWithEnvironment(config, config.variable.adminClientSubDomainName.value)}.${config.variable.domainName.value}`],

            comment: 'Admin Client Distribution',
            enabled: true,
            isIpv6Enabled: true,
            ...
            originAccessIdentities: {
                s3_bucket_one: 'Admin client distribution can access'
            },
            ...
            dependsOn: [
                config.variable.adminClientSubDomainName,
                config.variable.domainName,
        ...
        new Records(this, 'record', {
            ...
            records: [
                {
                    name: `${core.getSubDomainWithEnvironment(config, config.variable.adminClientSubDomainName.value)}`,
                    type: "A",
                    alias: {
                        name: adminCloudfront.cloudfrontDistributionDomainNameOutput,
                        zone_id: adminCloudfront.cloudfrontDistributionHostedZoneIdOutput
                    }
                }
            ],
